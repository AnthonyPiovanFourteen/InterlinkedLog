...
function RastreamentoPage() {
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Rastreamento"
        subtitle="Controle operacional. Acompanhe as cargas em trânsito e registre eventos."
      />
        ...
        {isLoading ? (
          ...
          (data ?? []).map((c) => {
            ...
            return (
              <Card
                key={c.contract_id}
                className="border-border/70 shadow-none cursor-pointer transition-colors hover:border-primary/40"
              ...
              >
                ...
                <CardContent className="text-xs space-y-2">
                  <div className="flex items-start gap-1.5">
                    ...
                    <div>
                      <p>{c.origin_city}</p>
                      <p className="text-muted-foreground">→ {c.destination_city}</p>
                    </div>
                  </div>
                  <div className="pt-1 text-muted-foreground border-t border-border">
                    ...
                    {delivery && (
                      <span
                        className={cn(
                          "ml-2 text-[10px] font-medium inline-flex items-center gap-0.5",
                          delivery.onTime ? "text-success" : "text-destructive",
                        )}
                      >
                        {delivery.onTime ? (
                          <>
                            <CheckCircle2 className="h-3.5 w-3.5 inline text-success" />{" "}
                            {delivery.days}/{delivery.deadline}d
                          </>
                        ) : (
                          <>
                            <AlertTriangle className="h-3.5 w-3.5 inline text-destructive" />{" "}
                            {delivery.days}/{delivery.deadline}d
                          </>
                        )}
                      </span>
          ...
          {selected && (
            <>
              <SheetHeader className="border-b border-border px-6 py-4 space-y-1">
                ...
                <SheetDescription>
                  {selected.carrier_name} · {selected.origin_city} → {selected.destination_city}
                </SheetDescription>
              </SheetHeader>
              <div className="px-6 py-5 overflow-y-auto h-[calc(100vh-80px)]">
                {(() => {
                  ...
                  return delivery ? (
                    <div
                      className={cn(
                        "rounded-md px-3 py-2 mb-5 text-sm",
                        delivery.onTime
                    ...
                    >
                      {delivery.onTime ? (
                        <p className="text-success font-medium">
                          <CheckCircle2 className="h-3.5 w-3.5 inline text-success" /> No prazo —
                          realizado em {delivery.days} dias (prometido: {delivery.deadline} dias)
                        </p>
                      ) : (
                        <p className="text-destructive font-medium">
                          <AlertTriangle className="h-3.5 w-3.5 inline text-destructive" /> Atraso
                          de {delivery.days - delivery.deadline} dias — realizado em {delivery.days}{" "}
                          dias (prometido: {delivery.deadline} dias)
                        </p>
                      )}
                    </div>
                  ) : (
                    <div className="rounded-md px-3 py-2 mb-5 text-sm bg-muted/50 border border-border">
                      <p className="text-muted-foreground">
                        ...
                        <span className="font-medium text-foreground">
                          {selected.deadline} dias
                        </span>{" "}
                        — aguardando eventos de coleta e entrega.
                      </p>
                    </div>
                  );
                })()}
                <ol className="relative space-y-5">
                  {selected.events.length === 0 ? (
                    ...
                    selected.events.map((e, i) => {
                      ...
                      const isOcorrencia =
                        e.title.toLowerCase().includes("ocorrência") ||
                        e.title.toLowerCase().includes("ocorrencia");
                      const isConcluded = isEntregue || isOcorrencia || !isLast;
                      const Icon2 = isOcorrencia
                        ? AlertTriangle
                        : isConcluded
                          ? CheckCircle2
                          : Clock;
                      const colorClass = isOcorrencia
                        ? "bg-destructive/15 border-destructive text-destructive"
                        : isConcluded
                          ? "bg-success/15 border-success text-success"
                          : "bg-info/15 border-info text-info animate-pulse";
                      return (
                        <li key={e.id ?? i} className="relative pl-9">
                          {!isLast && (
                            <span
                              className={cn(
                                "absolute left-[14px] top-7 bottom-[-22px] w-px",
                                isOcorrencia
                                  ? "bg-destructive/60"
                                  : isConcluded
                                    ? "bg-success/60"
                          ...
                          <div className="space-y-0.5">
                            ...
                            {e.observation && (
                              <p className="text-xs text-muted-foreground">{e.observation}</p>
                            )}
                          </div>
                        </li>
                      );
...
function AddEventButton({ contractId, onAdded }: { contractId: string; onAdded: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: () =>
      api.post(`/tracking/${contractId}/events`, {
  ...
  return (
    <>
      ...
      {open && (
        <div
          className="absolute right-6 top-14 z-50 bg-popover border border-border rounded-md shadow-lg p-3 w-72"
          onClick={(e) => e.stopPropagation()}
        >
          <label className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1 block">
            Status (selecione ou digite)
          </label>
          <select
          ...
          <div className="flex gap-1 justify-end">
            ...
            <Button
              size="sm"
              disabled={(!title && !customTitle) || mutation.isPending}
