...
function Resultado() {
  ...
  if (isLoading)
    return <div className="py-12 text-center text-muted-foreground">Carregando resultados...</div>;
  if (!data)
    return <div className="py-12 text-center text-muted-foreground">Cotação não encontrada.</div>;
  ...
  const melhorCB = [...resultados].sort(
    (a, b) => a.final_value / Math.max(1, a.deadline) - b.final_value / Math.max(1, b.deadline),
  )[0];
    ...
    <div className="space-y-5">
      ...
      {!isContractable && (
        <div className="rounded-md bg-muted/50 border border-border px-4 py-3 text-sm text-muted-foreground">
          ...
          e não aceita novas contratações.
        </div>
      )}
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <HighlightCard
          icon={Trophy}
          title="Melhor preço"
          value={melhorPreco?.carrier_name ?? "—"}
          hint={melhorPreco ? fmtCurr(melhorPreco.final_value) : ""}
          tone="success"
        />
        <HighlightCard
          icon={Zap}
          title="Melhor prazo"
          value={melhorPrazo?.carrier_name ?? "—"}
          hint={melhorPrazo ? `${melhorPrazo.deadline} dias úteis` : ""}
          tone="info"
        />
        <HighlightCard
          icon={Sparkles}
          title="Melhor custo-benefício"
          value={melhorCB?.carrier_name ?? "—"}
          hint={melhorCB ? `${fmtCurr(melhorCB.final_value)} · ${melhorCB.deadline}d` : ""}
          tone="primary"
        />
      </div>
              ...
              {resultadosComFlag.map((r) => (
                <TableRow key={r.carrier_id} className={cn(r.best_price && "bg-success/5")}>
                  <TableCell className="font-medium flex items-center gap-2">
                    ...
                    {r.best_deadline && !r.best_price && (
                      <Badge
                        variant="outline"
                        className="border-info/30 bg-info/10 text-info text-[10px]"
                      >
                        Melhor prazo
                      </Badge>
                    )}
                  ...
                  <TableCell className="text-right tabular-nums">
                    {fmtCurr(r.freight_value)}
                  </TableCell>
                  <TableCell className="text-right tabular-nums">{fmtCurr(r.fees)}</TableCell>
                  <TableCell className="text-right font-semibold tabular-nums">
                    {fmtCurr(r.final_value)}
                  </TableCell>
                  <TableCell className="text-right">
                    {isContractable ? (
                      <Button
                        size="sm"
                        variant={r.best_price ? "default" : "outline"}
                        onClick={() => contractMutation.mutate(r.carrier_id)}
