...
function CotacoesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => {
      ...
      return api
        .get<{ data: QuotationItem[] }>(`/quotations${qs ? `?${qs}` : ""}`)
        .then((r) => r.data);
    },
  });
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Cotações"
        subtitle="Lista de cotações realizadas."
        actions={
          <Button asChild>
      ...
      <Card className="border-border/70 shadow-none">
        <CardContent className="p-3 flex flex-wrap items-center gap-2">
          <Input
            placeholder="Buscar por NF, cidade..."
            value={search}
              ...
              {isLoading ? (
                ...
                (data ?? []).map((c) => (
                  <TableRow
                    ...
                    onClick={() => navigate({ to: "/cotacoes/resultado", search: { id: c.id } })}
                  >
