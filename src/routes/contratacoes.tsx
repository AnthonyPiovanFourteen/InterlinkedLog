...
function ContratacoesPage() {
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Contratações"
        subtitle="Fretes contratados a partir das cotações aprovadas."
      />
          ...
          {selected && (
            <>
              <SheetHeader className="border-b border-border px-6 py-4 space-y-1">
                ...
                <SheetDescription>
                  NF {selected.nf_number} · {selected.carrier_name}
                </SheetDescription>
              </SheetHeader>
              <div className="px-6 py-5 space-y-6 overflow-y-auto h-[calc(100vh-130px)]">
                <section>
                  ...
                  <dl className="grid grid-cols-2 gap-y-2.5 gap-x-4 text-sm">
                    ...
                    <dd>{selected.origin_city}</dd>
                    <dt className="text-muted-foreground">Destino</dt>
                    <dd>{selected.destination_city}</dd>
                    <dt className="text-muted-foreground">Valor</dt>
                    <dd className="font-semibold tabular-nums">{fmtCurr(selected.final_value)}</dd>
                    <dt className="text-muted-foreground">Prazo</dt>
                    <dd>{selected.deadline} dias</dd>
                ...
                <section>
                  ...
                  {selected.cte_number ? (
                    ...
                    <form
                      onSubmit={(e) => {
                        e.preventDefault();
                        if (cteInput) cteMutation.mutate({ id: selected.id, cte_number: cteInput });
                      }}
                      className="space-y-2"
                    >
                      <p className="text-xs text-muted-foreground">
                        Aguardando a transportadora informar o número do CT-e.
                      </p>
                      <div className="flex gap-2">
                        ...
                        <Button
                          type="submit"
                          size="sm"
                          disabled={cteMutation.isPending || !cteInput}
                        >
                          Registrar
                        </Button>
                      </div>
                ...
                <section>
                  ...
                  <form
                    onSubmit={(e) => {
                      ...
                      if (reason) cancelMutation.mutate({ id: selected.id, reason });
                    }}
                  >
                    <div className="flex gap-2">
                      <Input
